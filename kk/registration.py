...
class RegistrationForm(FlaskForm):
    ...
    def validate_email(self, email):
        existing_user = participants.query.filter_by(email=email.data).first()

        if existing_user:
            raise ValidationError('Email Address Already Used.')

    def validate_mobile(self, mobile):
        existing_user = participants.query.filter_by(
            mobile=mobile.data).first()
